-- ============================================================================
-- marketing_analysis.sql — performance by the proxies this dataset actually has
--
-- IMPORTANT SCOPE STATEMENT
-- -------------------------
-- This dataset contains NO marketing data: no campaigns, channels, ad spend,
-- impressions, clicks, sessions, referrers or UTM parameters. Nothing below is
-- channel attribution, and none of it can be used to compute CAC, ROAS or
-- campaign ROI.
--
-- What these queries do instead is measure performance across the dimensions
-- that DO exist and that a marketing team could act on: payment method, seller,
-- customer region, and category mix. They describe WHERE and HOW demand
-- converts, never WHERE IT CAME FROM.
--
-- To do real attribution you would need, at minimum: a session/visit table with
-- a channel or referrer per visit, campaign and spend data joined on campaign id,
-- and a customer-to-first-touch mapping. See the notebook 08 write-up.
-- ============================================================================


-- name: payment_method_performance
-- Payment type as a proxy for customer segment and purchasing power. Instalment
-- behaviour is the closest thing this dataset has to a "financing channel".
WITH order_payment AS (
    -- one dominant payment type per order: the method carrying the most value
    SELECT
        p.order_id,
        p.payment_type,
...
-- Does spreading payment associate with a larger basket? Instalments are only
-- available on credit cards, so the comparison is held within credit card orders.
WITH order_payment AS (
    SELECT
        p.order_id,
        p.payment_type,
...
-- Region as an acquisition territory: volume, value, delivery experience and
-- satisfaction side by side, so a "good" market is judged on more than revenue.
...
-- Sellers as a distribution "channel": who moves volume, at what price point,
-- with what delivery reliability. The closest available analogue to comparing
-- partners or channels.
...
-- Which category leads in each state — a targeting input for regional
-- messaging. Uses a partitioned rank to pick each state's top category.
...
-- The one "marketing" question this data CAN answer well: does a good delivery
-- experience associate with the customer coming back? Observational only.
