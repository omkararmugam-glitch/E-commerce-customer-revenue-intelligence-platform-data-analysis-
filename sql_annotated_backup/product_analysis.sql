-- ============================================================================
-- product_analysis.sql — product and category performance
--
-- NOTE ON SCOPE: this dataset carries no cost of goods, so gross margin,
-- markup and profit-based quadrant analysis are NOT computable. Every
-- "performance" measure below is revenue, price or volume based, and is
-- labelled as such.
-- ============================================================================


-- name: top_products_by_revenue
-- Best sellers with their category rank alongside their overall rank, so a
-- product that leads its category but not the catalogue is still visible.
...
-- The other end of the catalogue: products that sold once, at a low price.
-- Volume-and-revenue based only — without cost data we cannot say whether a
-- low-revenue product is unprofitable, only that it is not selling.
WITH product_perf AS (
    SELECT
        i.product_id,
        i.category,
...
-- Formal Pareto test on categories: cumulative revenue share ordered by rank,
-- with the 80% cut marked.
...
-- Revenue-vs-volume quadrant. This REPLACES the usual margin-vs-volume
-- quadrant, which cannot be built here: no cost data exists in this dataset.
-- Categories are classified against the median category on both axes.
...
-- Median of each axis, computed with window functions so category_stats is
-- scanned once. (A correlated "ORDER BY ... LIMIT 1 OFFSET (SELECT COUNT(*)...)"
-- re-evaluates the CTE for every probe and is far slower.)
positioned AS (
    SELECT
        avg_price,
        units,
...
-- How revenue splits across price bands. Shows that a small share of high-price
-- lines carries a disproportionate share of revenue — the finding that made the
-- Phase 3 decision not to trim price outliers.
SELECT
    CASE
...
-- Which categories are growing and which are shrinking: the last three months
-- of the window against the three before them.
WITH recent AS (
    SELECT
        category,
...
-- Revenue concentration across sellers — a marketplace risk measure.
WITH seller_perf AS (
    SELECT
        seller_id,
        seller_state,
